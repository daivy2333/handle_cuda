...
protected:
    ...
    std::vector<float> sigmoid_ref(const std::vector<float>& input) {
    ...
    std::vector<float> sigmoid_backward_ref(const std::vector<float>& grad_out,
                                            const std::vector<float>& forward_output) {
        std::vector<float> grad_in(grad_out.size());
        for (size_t i = 0; i < grad_out.size(); ++i) {
            grad_in[i] = grad_out[i] * forward_output[i] * (1.0f - forward_output[i]);
        }
        return grad_in;
    }
};

TEST_F(SigmoidTest, BasicForward) {
    size_t size = 1024;
    auto input = generate_random(size);
    auto expected = sigmoid_ref(input);

    CudaBuffer d_input(size), d_output(size);
    host_to_device_async(d_input.data, input.data(), size);

    cuda_sigmoid(d_input.data, d_output.data, size);
...
TEST_F(SigmoidTest, BasicBackward) {
    size_t size = 1024;
    auto forward_input = generate_random(size);
    auto forward_output = sigmoid_ref(forward_input);
    auto grad_out = generate_random(size);
    auto expected = sigmoid_backward_ref(grad_out, forward_output);
    ...
    cuda_sigmoid_backward(d_grad_out.data, d_forward_output.data, d_grad_in.data, size);
